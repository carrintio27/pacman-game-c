...
void ghost_destory(Ghost* ghost) {
	...
	if (ghost->move_sprite != NULL) {
		al_destroy_bitmap(ghost->move_sprite);
	}
	if (ghost->flee_sprite != NULL) {
		al_destroy_bitmap(ghost->flee_sprite);
	}
	if (ghost->dead_sprite != NULL) {
...
void ghost_draw(Ghost* ghost) {
	...
	if (ghost->objData.moveCD % 32 >= 0 && ghost->objData.moveCD % 32 <= 15) {
		bitmap_x_offset = 0;
	}
	else if (ghost->objData.moveCD % 32 >= 16 && ghost->objData.moveCD % 32 <= 32) {
		bitmap_x_offset = 16;
	}
	if (ghost->status == FLEE) {
		...
		*/ 
		float remaining_percentage = (float)get_power_up_timer_tick() / (float)get_power_up_duration();
			if (remaining_percentage * 100 >= 70.0)
			{
...
bool ghost_movable(const Ghost* ghost, const Map* M, Directions targetDirec, bool room) {
	...
	int ghx = ghost->objData.Coord.x; int ghy = ghost->objData.Coord.y;
	...
	if (is_wall_block(M, ghx, ghy) || (room && is_room_block(M, ghx, ghy)))
		return false;
