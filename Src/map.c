...
void delete_map(Map* M) {
	...
	// Free dynamically allocated memory within the Map structure
	if (M->map) {
		// Assuming M->map is a dynamically allocated 2D array
		for (int i = 0; i < M->row_num; ++i) {
			free(M->map[i]); // Free each row
		}
		free(M->map); // Free the array of rows
	}

	// TODO-GC-memory: free any other dynamically allocated members of Map here
