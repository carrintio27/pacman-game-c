...
char pacman_sprite_move[30] = "Assets/pacman_move.png";
char pacman_sprite_die[30] = "Assets/pacman_die.png";
...
static bool pacman_movable(const Pacman* pacman, const Map* M, Directions targetDirec) {
	...
	int pcx = pacman->objData.Coord.x;  int pcy = pacman->objData.Coord.y;
	...
	if (is_wall_block(M, pcx, pcy) || is_room_block(M, pcx, pcy))
		return false;
...
Pacman* pacman_create() {
	...
	pman->powerUp = false;
	/* load sprites */
	pman->move_sprite = load_bitmap(pacman_sprite_move);
	pman->die_sprite = load_bitmap(pacman_sprite_die);
	return pman;

}

void change_color(char* movefile, char* diefile)
{
	strcpy_s(pacman_sprite_move, sizeof(pacman_sprite_move), movefile);
	strcpy_s(pacman_sprite_die, sizeof(pacman_sprite_die), diefile);
}
...
void pacman_draw(Pacman* pman) {
	...
	if (!game_over) {
		...
		if (pman->objData.moveCD % 32 >= 0 && pman->objData.moveCD % 32 <= 15) {
			offset = 0;
		}
		else if(pman->objData.moveCD %32 >= 16 && pman->objData.moveCD % 32 <= 32){
			offset = 16;
		}
		...
		*/
		...
		long long counter = al_get_timer_count(pman->death_anim_counter);
		al_draw_scaled_bitmap(pman->die_sprite, 16*counter, 0,
			16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0
		);
...
void pacman_move(Pacman* pacman, Map* M, bool pacwall) {
	...
	if (pacman_movable(pacman, M, pacman->objData.nextTryMove) || pacwall) 
