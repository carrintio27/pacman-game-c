...
static ALLEGRO_BITMAP* background1 = NULL;
static int background1W;
static int background1H;
...
int UpButton = ALLEGRO_KEY_W;
int DownButton = ALLEGRO_KEY_S;
int LeftButton = ALLEGRO_KEY_A;
int RightButton = ALLEGRO_KEY_D;

char selected_map[30] = "Assets/map_nthu.txt";

void change_map(char* mapfilename)
{
	strcpy_s(selected_map, sizeof(selected_map), mapfilename);
}

static void init(void) {
	if(pickbg == 1) background1 = load_bitmap("Assets/background1.png");
	else if(pickbg == 2) background1 = load_bitmap("Assets/background2.png");
	else if(pickbg == 3) background1 = load_bitmap("Assets/background3.png");
	
	background1W = al_get_bitmap_width(background1);
	background1H = al_get_bitmap_height(background1);
	...
	basic_map = create_map(selected_map);
	if (!basic_map) {
...
static void status_update(void) {
	...
	for (int i = 0; i < GHOST_NUM; i++) {
		...
		else if (ghosts[i]->status == FREEDOM)
		{
			...
			RecArea pacmanDrawArea = getDrawArea((object*)pman, GAME_TICK_CD);

			RecArea ghostDrawArea = getDrawArea((object*)ghosts[i], GAME_TICK_CD);

			if(RecAreaOverlap(&pacmanDrawArea, &ghostDrawArea) && !cheat_mode)
			{
...
static void update(void) {
	...
	checkItem();
	status_update();
	pacman_move(pman, basic_map, pacwall);
	for (int i = 0; i < GHOST_NUM; i++) 
...
static void draw(void) {
	...
	al_draw_textf(menuFont, al_map_rgb(255, 255, 255), SCREEN_W - 500, 10, ALLEGRO_ALIGN_LEFT, "Score: %d", game_main_Score);
...
static void on_key_down(int key_code) {
	...
	if (key_code == UpButton) {
		pacman_NextMove(pman, UP);
	}
	else if (key_code == LeftButton) {
		pacman_NextMove(pman, LEFT);
	}
	else if (key_code == DownButton) {
		pacman_NextMove(pman, DOWN);
	}
	else if (key_code == RightButton) {
...
void change_controls(int keytochange, int new_value) {
	if (keytochange == 0) {
		printf("changes to %d" , new_value);
		UpButton = new_value;
	}
	else if (keytochange == 2) {
		printf("changes to %d", new_value);
		 LeftButton= new_value;
	}
	else if (keytochange == 1) {
		printf("changes to %d", new_value);
		DownButton = new_value;
	}
	else if (keytochange == 3) {
		printf("changes to %d", new_value);
		RightButton = new_value;
	}
